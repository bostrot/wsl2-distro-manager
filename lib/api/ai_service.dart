...
Creating a Linux VM: it MUST boot from something — a blank disk boots into nothing and stops immediately. Always give vm_create_linux a boot source: pass a "catalog" id (call vm_list_images first) or an image_path/iso_path. Prefer a cloud image / catalog entry when the user wants a usable system, because a cloud-init seed makes it reachable over SSH with no manual install — then wsl_run_command and service recipes work. Do NOT create a VM with no boot source and then try to start it; the tool now refuses that.
